...
size_t const PsFreeChunk::header_size() {
  return sizeof(PsFreeChunk)/HeapWordSize;
}

void PsFreeChunk::mangleAllocated(size_t size) {
  ...
  assert(sizeof(PsFreeChunk) % HeapWordSize == 0,
...
void PsFreeChunk::mangleFreed(size_t sz) {
...
void PsFreeChunk::verifyList() const {
  PsFreeChunk* nextFC = next();
  if (nextFC != NULL) {
...
void PsFreeChunk::print_on(outputStream* st) {
