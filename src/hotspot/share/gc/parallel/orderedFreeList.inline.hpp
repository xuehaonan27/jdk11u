...
void OrderedFreeList<Chunk>::insert_chunk(Chunk*fc) {
    ...
    Chunk* _tail = tail();
    Chunk* _finger = _head;
    Chunk* _insert_point = NULL;

    while(_finger){
        if(fc < _finger){
            _insert_point = _finger->prev();
            break;
        }
    }

    if(!_finger){
        //Should be inserted to tail
        if(_tail == NULL){
            link_tail(fc);
            link_head(fc);
        }
        ...
        if(!_insert_point){
            ...
            _insert_point->link_next(fc);
            fc->link_next(_finger);
            fc->link_prev(_insert_point);
            _finger->link_prev(fc);
        }
    }
